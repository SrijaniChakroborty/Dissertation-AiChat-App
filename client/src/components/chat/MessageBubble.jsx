...
const MessageBubble = ({ props }) => {
  ...
  console.log(codeBlockMatch ? codeBlockMatch.input : "null");

  const textBeforeCode = codeBlockMatch
    ? messageText.split(/```(\w+)\s*[\s\S]*?\s*```/)[0]
    : "";
  console.log(textBeforeCode);
  const textAfterCode = codeBlockMatch
    ? messageText.split(/```(\w+)\s*[\s\S]*?\s*```/).pop()
    : "";
  ...
  useEffect(() => {
    ...
    // if(props.message.attachments[0].file.includes('.docx') || props.message.attachments[0].file.includes('.doc')){}
  ...
  const renderChatDate = () => {
    const chatDate = new Date(props.message.created);
    const options = { year: 'numeric', month: 'long', day: 'numeric' };
    return (
      <div className="chat-date">
        {chatDate.toLocaleDateString(undefined, options)}
      </div>
    );
  };
  ...
  return props.isMyMessage ? (
    !codeBlockMatch ? (
      <div className="my-message-container">
        <div className="my-message">
          {props.message.attachments.length != 0 && !props.message.attachments[0].file.includes('.docx') ? (
          ...
          props.message.attachments[0].file.includes(".docx") ? (
          !isLoading ? (
            <div
    ...
    <div className="other-message-container">
      <div className="other-message">
        {renderAvatar()}
        {props.message.attachments.length != 0 && !props.message.attachments[0].file.includes('.doc') ? (
          <img
            className="my-message-image"
            src={props.message.attachments[0].file}
          />
        ...
        !isLoading ? (
          <div
            className="pdf-message other-pdf-message"
            onClick={() => {
              window.open(props.message.attachments[0].file, "_blank");
            }}
          >
    ...
    <div className="code-message-container">
      <div className="code-avatar-container">
        {renderAvatar()}
        <div className="code-text-message-container">
          <div className="code-pre-message">
          {textBeforeCode && (
            <p className="other-text-content">{textBeforeCode}</p>
          )}
          </div>
          <div className="other-message">
          {codeBlockMatch[1] === "pdf" ? ( //pdf file bubble
      ...
      <div className="code-pro-message">
      {textAfterCode && <p className="other-text-content">{textAfterCode}</p>}
      </div>
      {renderTimestamp()}
